...
use legion::storage::{Archetype, ComponentTypeId};

use crate::{component::ExternalComponent, driver::convert_bytes_into_pointer};
use std::os::raw::c_void;
use std::slice;

use log::*;

fn get_component_from_storage(
    ...
    let (slice_ptr, _len) = storage
        .get_raw(archetype.index())
        .expect("Failed to get raw component");
...
pub fn get_external_components(
    world: &legion::world::World,
    component_type_ids: Vec<ComponentTypeId>,
    components: &mut Vec<*const c_void>,
) {
